...
func buildRangeQueries(Metrics []string, StartTime, EndTime string) map[string]string {
	...
	if contains(Metrics, "HTTPRequestCount") {
		MetricQuery["HTTPRequestCount"] = fmt.Sprintf("http://%v:%v/api/v1/query_range?query=sum(irate(app_http_request_count[1m]))&start=%v&end=%v&step=2", domain, port, StartTime, EndTime)
	}

	if contains(Metrics, "HTTPRequestLatency") {
		MetricQuery["HTTPRequestLatency"] = fmt.Sprintf("http://%v:%v/api/v1/query_range?query=app_http_request_latency&start=%s&end=%s&step=2", domain, port, StartTime, EndTime)
	}

	if contains(Metrics, "IOWait") {
		MetricQuery["IOWait"] = fmt.Sprintf("http://%v:%v/api/v1/query_range?query=avg(irate(node_cpu{job='node-exporter',mode='iowait'}[1m]))*100&start=%v&end=%v&step=2", domain, port, StartTime, EndTime)
	}

	if contains(Metrics, "MemoryUsage") {
		MetricQuery["MemoryUsage"] = fmt.Sprintf("http://%v:%v/api/v1/query_range?query=((node_memory_MemTotal)%%20-%%20((node_memory_MemFree%%2Bnode_memory_Buffers%%2Bnode_memory_Cached)))%%20%%2F%%20node_memory_MemTotal%%20*%%20100&start=%v&end=%v&step=2", domain, port, StartTime, EndTime)
	}

	if contains(Metrics, "WriteTime") {
		MetricQuery["WriteTime"] = fmt.Sprintf("http://%v:%v/api/v1/query_range?query=irate(node_disk_sectors_written[5m])*512&start=%v&end=%v&step=2", domain, port, StartTime, EndTime)
	}

	if contains(Metrics, "ReadTime") {
		MetricQuery["ReadTime"] = fmt.Sprintf("http://%v:%v/api/v1/query_range?query=irate(node_disk_sectors_read[5m])*512&start=%v&end=%v&step=2", domain, port, StartTime, EndTime)
	}

	if contains(Metrics, "CPUUsage") {
		MetricQuery["CPUUsage"] = fmt.Sprintf("http://%v:%v/api/v1/query_range?query=100-(avg%%20by%%20(instance)%%20(irate(node_cpu{job='node-exporter',mode='idle'}[1m]))*100)&start=%v&end=%v&step=2", domain, port, StartTime, EndTime)
	}
...
func buildDataset(MetricStructs map[string]RangePrometheusJSON) ([]string, map[int]interface{}) {
...
func writeToCSV(knobs map[string]int, dataset map[int]interface{}, featureNames []string, fileName string) {
...
func saveDataset(knobs map[string]int, dataset map[int]interface{}, featureNames []string) {
