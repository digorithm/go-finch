...
SELECT * FROM RECIPE R INNER JOIN HOUSE_RECIPE H ON R.ID = H.RECIPE_ID WHERE H.HOUSE_ID = %s
#GetHouseSchedule(house)
...
#GetOwnDescription(own_id)
SELECT O.DESCRIPTION FROM OWNERSHIP O WHERE O.OWN_TYPE = %v
#-------------------------------SET------------------------------#
#AddUser
...
#-------------------------------GET------------------------------#
#GetHouses (user)
SELECT H.NAME, H.ID, O.OWN_TYPE, O.DESCRIPTION FROM HOUSE H INNER JOIN MEMBER_OF M ON M.HOUSE_ID = H.ID 
INNER JOIN OWNERSHIP O ON O.OWN_TYPE = M.OWN_TYPE WHERE M.USER_ID = %s
Alternative: (SELECT * FROM HOUSE H INNER JOIN MEMBER_OF M ON M.HOUSE_ID = H.ID WHERE M.USER_ID = %s)
#GetUserRecipes (user)
SELECT * FROM RECIPE R INNER JOIN USER_RECIPE U ON R.ID = U.RECIPE_ID WHERE R.ID = %s
